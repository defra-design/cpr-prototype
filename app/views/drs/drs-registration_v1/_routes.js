...
router.post('*/route-check-defra', function (req, res) {
    ...
    if (daChoice == "yes"){
        res.redirect('/drs/defra-id/login')
    }
    else {
...
router.post('*/route-reporting', function (req, res) {
    var orgDescription = req.session.data['orgDescription']
    if (orgDescription.includes('producer')){
        res.redirect('../drs-reporting_v1/report-producer-start')
    }
    else if (orgDescription.includes('producer')){
        res.redirect('report-producer-start')
    }
